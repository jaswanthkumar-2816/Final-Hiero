...
async function submitStep(step) {
  ...
  try {
    ...
    const response = await fetch(`/api/resume/${step}`, {
      method: 'POST',
      headers,
...
async function generateResume() {
  try {
    ...
    if (selectedTemplate) {
      await fetch('/api/resume/template', {
        method: 'POST',
        headers,
    ...
    const response = await fetch('/api/resume/generate-fast', {
      method: 'POST',
      headers
    });
    ...
    if (response.ok && result.success) {
      ...
      // Download the file
      if (result.file) {
        window.location.href = `/api/resume/download?file=${result.file}`;
      }
    } else {
