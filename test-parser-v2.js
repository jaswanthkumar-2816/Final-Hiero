...
function parseResumeText(rawText) {
    ...
    const websiteMatch = text.match(/(?:https?:\/\/)?(?:www\.)?[\w-]+\.(?:com|org|net|io|me|dev)(?:\/[\w-]+)*/i);
    if (websiteMatch && !websiteMatch[0].includes('linkedin') && !websiteMatch[0].includes('gmail') && !websiteMatch[0].includes('email')) {
        data.personalInfo.website = websiteMatch[0];
    }

    // --- Address Extraction ---
    const addressMatch = text.match(/[A-Z][a-z]+, [A-Z]{2}(?: \d{5})?|[A-Z][a-z]+, [A-Z][a-z]+/);
    if (addressMatch) data.personalInfo.address = addressMatch[0];
    ...
    // --- 3. Sectioning ---
    const sections = { experience: [], education: [], skills: [], projects: [], summary: [], certifications: [], achievements: [], languages: [], hobbies: [], fallback: [] };
    ...
    cleanLines.forEach(line => {
        ...
        if (!isHeader) {
            if (currentSection) sections[currentSection].push(line);
                ...
                // Check if this line looks like contact info we already have
    ...
    if (sections.skills.length > 0) {
        data.technicalSkills = sections.skills.map(s => s.replace(/^[\s•\-\+\*#>]+\s*/, '').trim()).join(', ');
    }
    data.achievements = sections.achievements.join('\n').trim();
    data.hobbies = sections.hobbies.join(', ').trim();
    data.languages = sections.languages.map(s => s.replace(/^[\s•\-\+\*#>]+\s*/, '').trim());
    data.certifications = sections.certifications.map(s => s.replace(/^[\s•\-\+\*#>]+\s*/, '').trim());
    ...
    sections.projects.forEach(line => {
        const clean = line.replace(/^[\s•\-\+\*#>]+\s*/, '').trim();
        const isBullet = /^[\s•\-\+\*]/.test(line);
        if (!projItem || (clean.length < 50 && !isBullet && !clean.match(/[.,]/))) {
            if (projItem) data.projects.push(projItem);
            projItem = { name: clean, description: '', tech: '' };
        } else {
            projItem.description += (projItem.description ? '\n' : '') + clean;
        }
    });
    if (projItem) data.projects.push(projItem);

    data.experience = data.experience.filter(item => item.jobTitle !== 'Professional Role' || item.company || item.startDate);
    data.education = data.education.filter(item => item.school !== 'Educational Institution' || item.degree);
    data.projects = data.projects.filter(item => item.name && item.name.length > 2);

    if (!data.personalInfo.fullName && cleanLines.length > 0) {
        data.personalInfo.fullName = cleanLines[0].replace(/[^a-zA-Z\s]/g, '').trim();
    }
