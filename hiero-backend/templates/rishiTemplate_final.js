export function generateRishiTemplate(data) {
  ...
  const phone = personalInfo?.phone || '+91 9975808780';
  ...
  const projectsList = Array.isArray(projects) && projects.length > 0
    ...
    : [
        {
          title: 'Dynamic Analysis of Buckling Restrained Braces',
          description: 'The project aims at designing and fabrication of two Buckling Restrained Braces which were analyzed under dynamic loading. As alternative for conventional braces, these BRBs are also beneficial for seismic retro-fitting in RCC and steel structures.'
        },
        {
          title: 'Indirect Model Analysis of Structures',
          description: 'Presented a Seminar on Indirect Model Analysis, explaining the method to compute response of Prototype from the Influence lines obtained from Model. Use of Muller Breslau Principle in Indirect Model Analysis and the Similitude between prototype and model.'
        },
        {
          title: 'Microtunneling',
          description: 'Presented a seminar on Micro Tunneling, explaining its advantages over conventional method of drainage laying systems. Analysis considering direct and indirect cost of micro tunneling was also discussed.'
        }
      ];
  ...
  const experienceList = Array.isArray(experience) && experience.length > 0
    ...
    : [
        {
          company: 'SJ Contracts, Pune',
          date: 'June 2016',
          role: 'Site Engineer',
          points: ['On-site internship under this leading construction company. Learned and implemented various aspects such as quantity estimation, labour management and safety precautions.']
        }
      ];

  // === ACADEMIC ACHIEVEMENTS ===
  const achievementsList = Array.isArray(achievements) && achievements.length > 0
    ? (typeof achievements[0] === 'string' ? achievements : achievements.map(a => a.text || a.description || ''))
    : [
  ...
  const extraList = Array.isArray(extracurricular) && extracurricular.length > 0
    ? (typeof extracurricular[0] === 'string' ? extracurricular : extracurricular.map(e => e.text || e.activity || ''))
    : [
  ...
  const traitsList = Array.isArray(traits) && traits.length > 0
    ? (typeof traits[0] === 'string' ? traits : traits.map(t => t.text || t.trait || ''))
    : [
...
<div class="resume">
  ...
  <div class="section">
    <table class="skills-table">
      <tr>
        <td class="label">Modeling and Analysis</td>
        <td>${modeling}</td>
      </tr>
      <tr>
        <td class="label">Software &amp; Tools</td>
        <td>${software}</td>
      </tr>
    </table>
  </div>
