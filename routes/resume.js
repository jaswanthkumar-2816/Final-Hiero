...
function generateWordHTML(data) {
    ...
    if (template === 'hiero-vertex' || template === 'vertex') {
        const BG_TOP = '#F8FAFC';
        const BG_LEFT = '#FFFFFF';
        const BG_RIGHT = '#0F172A';
        const TEXT_DARK = '#0F172A';
        const TEXT_LIGHT = '#F8FAFC';
        const ACCENT_DARK = '#94A3B8';
        const BOX_BG = '#E2E8F0';

        let html = '';

        // Header
        const fullName = (personalInfo.fullName || data.name || 'LOTHER SMITH').toUpperCase();
        const roleTitle = (personalInfo.roleTitle || data.title || 'DESIGNER & DEVELOPER').toUpperCase();

        html += `
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: ${BG_TOP}; text-align: center; padding: 40pt;">
            <tr><td>
                <div style="font-size: 32pt; font-family: 'Helvetica', sans-serif; font-weight: bold; color: ${TEXT_DARK}; letter-spacing: 2pt;">${fullName}</div>
                <div style="font-size: 12pt; font-family: 'Helvetica', sans-serif; color: ${TEXT_DARK}; letter-spacing: 4pt; margin-top: 10pt;">${roleTitle}</div>
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 25pt; border: 1pt solid ${ACCENT_DARK};">
                    <tr>
                        <td align="center" style="padding: 10pt; font-family: 'Helvetica', sans-serif; font-size: 9pt; color: ${TEXT_DARK}; border-right: 1pt solid ${ACCENT_DARK}; width: 33%;">
                            <span style="font-size: 14pt;">&#9742;</span><br/>
                            <span style="font-size: 9pt; margin-top: 4pt; display: block;">${personalInfo.phone || '+(000) 6666 6666'}</span>
                        </td>
                        <td align="center" style="padding: 10pt; font-family: 'Helvetica', sans-serif; font-size: 9pt; color: ${TEXT_DARK}; border-right: 1pt solid ${ACCENT_DARK}; width: 33%;">
                            <span style="font-size: 14pt;">&#9993;</span><br/>
                            <span style="font-size: 9pt; margin-top: 4pt; display: block;">${personalInfo.email || 'yourname@gmail.com'}</span>
                        </td>
                        <td align="center" style="padding: 10pt; font-family: 'Helvetica', sans-serif; font-size: 9pt; color: ${TEXT_DARK}; width: 33%;">
                            <span style="font-size: 14pt;">&#128205;</span><br/>
                            <span style="font-size: 9pt; margin-top: 4pt; display: block;">${personalInfo.address || 'Washington, United States'}</span>
                        </td>
                    </tr>
                </table>
            </td></tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                <td width="60%" valign="top" style="background-color: ${BG_LEFT}; padding: 30pt;">
        `;

        const renderLeftHeader = (title) => `
            <div style="font-size: 14pt; font-family: 'Helvetica', sans-serif; font-weight: bold; color: ${TEXT_DARK}; letter-spacing: 1.5pt; text-transform: uppercase;">${title}</div>
            <div style="height: 1pt; background-color: ${ACCENT_DARK}; margin-top: 8pt; margin-bottom: 12pt;"></div>
        `;

        if (summary) {
            html += renderLeftHeader('STATEMENT');
            html += `<div style="font-size: 9.5pt; font-family: 'Helvetica', sans-serif; color: ${TEXT_DARK}; text-align: justify; line-height: 1.5; margin-bottom: 25pt;">${summary}</div>`;
        }

        if (experience && experience.length > 0) {
            html += renderLeftHeader('EXPERIENCE');
            experience.forEach(exp => {
                ...
                const desc = exp.description || 'Lorem ipsum dolor sit amet...';

                html += `
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 20pt;">
                    <tr>
                        <td width="80" valign="top">
                            <div style="background-color: ${BOX_BG}; padding: 4pt; text-align: center; font-size: 8.5pt; font-family: 'Helvetica', sans-serif; color: ${TEXT_DARK};">${dateStr}</div>
                        </td>
                        <td valign="top" style="padding-left: 15pt;">
                            <div style="font-size: 10pt; font-family: 'Helvetica', sans-serif; font-weight: bold; color: ${TEXT_DARK};">${title}</div>
                            <div style="font-size: 9pt; font-family: 'Helvetica', sans-serif; color: ${TEXT_DARK}; margin-top: 2pt;">${compLoc}</div>
                            <div style="font-size: 9.5pt; font-family: 'Helvetica', sans-serif; color: ${TEXT_DARK}; text-align: justify; line-height: 1.5; margin-top: 10pt;">${desc}</div>
                        </td>
                    </tr>
                </table>
                `;
        ...
        html += `
                ...
                <td width="40%" valign="top" style="background-color: ${BG_RIGHT}; padding: 30pt;">
        `;

        const renderRightHeader = (title) => `
            <div style="font-size: 12pt; font-family: 'Helvetica', sans-serif; font-weight: bold; color: ${TEXT_LIGHT}; letter-spacing: 1.5pt; text-transform: uppercase;">${title}</div>
            <div style="height: 1pt; background-color: ${TEXT_LIGHT}; margin-top: 8pt; margin-bottom: 15pt;"></div>
        `;

        if (education && education.length > 0) {
            html += renderRightHeader('EDUCATION');
            education.forEach(edu => {
                ...
                <div style="margin-bottom: 18pt;">
                    <div style="font-size: 10pt; font-family: 'Helvetica', sans-serif; font-weight: bold; color: ${TEXT_LIGHT};">${title}</div>
                    <div style="font-size: 9pt; font-family: 'Helvetica', sans-serif; color: #A3A3A3; margin-top: 4pt;">${inst}</div>
                    <div style="font-size: 9pt; font-family: 'Helvetica', sans-serif; color: #A3A3A3; margin-top: 2pt;">${dateStr}</div>
                </div>
                `;
            });
        }

        let interestsRaw = data.hobbies || data.interests || 'Traveling, Playing, Swimming, Watching';
        if (interestsRaw) {
            html += renderRightHeader('INTERESTS');
            html += `<div style="font-size: 9.5pt; font-family: 'Helvetica', sans-serif; color: #A3A3A3; margin-bottom: 25pt;">${interestsRaw}</div>`;
        }

        const renderSkills = (title, items) => {
            if (!items || items.length === 0) return '';
            let sHtml = renderRightHeader(title);
            items.slice(0, 5).forEach((skill) => {
                const name = typeof skill === 'string' ? skill : (skill.name || skill.skill || 'Skill');
                sHtml += `<div style="font-size: 9.5pt; font-family: 'Helvetica', sans-serif; color: #A3A3A3; margin-bottom: 12pt;">• ${name}</div>`;
            });
            return sHtml;
        };

        const tSkills = data.skills || ['Photoshop', 'Illustrator', 'Indesign', 'Ms Word', 'Powerpoint'];
        const sSkills = data.softSkills || ['Creative', 'Teamwork', 'Punctual', 'Leadership'];
        html += renderSkills('PRO SKILLS', tSkills);
        html += renderSkills('PER SKILLS', sSkills);
