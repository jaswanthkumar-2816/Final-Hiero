    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            switch (template) {
                ...
                case 'hiero-axis':
                    await renderTemplate_HieroAxis(doc, data, colors, spacing);
                    doc.end();
                    if (outStream && outStream.on) {
                        outStream.on('finish', () => resolve(true));
                        outStream.on('error', reject);
...
/**
 * HIERO AXIS - Warm earthy sidebar layout with brown & beige palette
 */
async function renderTemplate_HieroAxis(doc, rawData, colors, spacing) {
    const data = normalizeData(rawData);
    const sidebarWidth = PAGE_CONFIG.width * 0.32;
    const contentWidth = PAGE_CONFIG.width - sidebarWidth;
    const pInfo = data.personalInfo || {};

    // Axis warm brown / beige palette
    const COLORS = {
        sidebar: '#8D6E63',
        main: '#F4F1EC',
        accent: '#8D6E63',
